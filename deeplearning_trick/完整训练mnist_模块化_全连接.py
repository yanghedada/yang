...
def batch_norm(x, name_scope, training, epsilon=1e-3, decay=0.99):  
    with tf.variable_scope(name_scope):  
        ...
        offset = scale = tf.get_variable('scale', [size], initializer=tf.constant_initializer(0.1))    
...
def batch_norm(x ,training, epsilon=1e-3, decay=0.99):  
    axis = list(range(len(x.get_shape()) - 1))
    size = x.get_shape().as_list()[-1]  
    scale = tf.Variable(tf.random_normal([size])) 
    offset = tf.Variable(tf.random_normal([size]))  
    fc_mean, fc_var = tf.nn.moments(x, axis )
    ema = tf.train.ExponentialMovingAverage(decay=0.99)
